...
- **Definition of Done (mirrored verbatim in `.github/workflows/ci.yml`, CI is the authoritative gate):**
  ...
  - **Test + Build:** `xcodebuild test -scheme Scout -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO` — `xcodebuild test` compiles first, so the build step is folded into it. Simulator builds need **no code signing**. Adjust the simulator name to one your Xcode / the CI runner actually lists (`xcrun simctl list devices`).
