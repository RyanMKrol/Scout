...
- **Scout** is a **SwiftUI iOS app (minimum deployment target iOS 26)** that measures **live
  cellular data throughput** — it forces a data transfer over the `.cellular` interface
  (`Network.framework`'s `NetworkConnection`), computes a rolling-window Mbps figure updated ~4×/sec,
  and tags it with the current radio generation (5G / LTE) via `CoreTelephony`. You sweep the phone
  around a room like a metal detector to find the spot with the best signal. It deliberately measures
  *throughput*, not raw dBm/bars — no App-Store-safe iOS app can read radio signal strength.
...
- **The `README.md` is a product document** — what the project IS and how a human uses it. It is
  **maintainer-owned**: the loop NEVER edits it, and it does **not** track implementation status
  (that lives in `TASKS.json` + the dashboard, always current; a status section in the README would
  only go stale). Keeping the README accurate is a deliberate human decision, not a per-change chore.
...
5. If working BY HAND (no loop running), set the task's `TASKS.json` `"status"` in the same commit —
   under the harness the loop owns status, so leave it alone (golden rule 3). The root `README.md` is
   maintainer-owned product documentation, not a status log — don't update it per change (golden rule 3).
...
- **Stack:** Swift 6 / SwiftUI, iOS 26 minimum, built with Xcode (`xcodebuild`). No SwiftPM
  dependencies planned for v1 — first-party frameworks only (`Network`, `CoreTelephony`, `SwiftUI`).
- **The `Scout.xcodeproj` is GENERATED by XcodeGen from `project.yml` and is git-ignored — never
...
- **Testing convention:** XCTest (not Swift Testing, for consistency with the team's sibling harness
  ...
  never runs in CI. Unit-test the **pure math** (rolling-window Mbps, 5G/LTE tagging) from the fake, with
  an **injected clock** (never bare `Date()`/`.now`) so the ~4Hz windowing is deterministic. **UI-test
  house rules:** query by `accessibilityIdentifier`, never display copy; use bounded waits; never assert
  live UI state directly — give the Mbps readout, radio badge, and start/stop control stable identifiers.
